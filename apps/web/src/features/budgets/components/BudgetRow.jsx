...
export default function BudgetRow({ row, currency = 'INR', onEdit }) {
  ...
  return (
    <li className="py-3.5 first:pt-0 last:pb-0">
      <div className="mb-2 flex items-center gap-2.5">
        ...
        {onEdit && (
          <button
            type="button"
            onClick={() => onEdit(row)}
            aria-label={t('budget.editAria', { category: label(row.category) })}
      ...
      <ProgressBar value={row.usedPercent} tone={row.status} label={t('budget.usedAria', { category: label(row.category) })} />
